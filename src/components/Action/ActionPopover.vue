...
const clickOutsideEvent = (e: MouseEvent) => {
  ...
  if ((document.querySelector('.bookmark-config-dialog') as any)?.contains(e.target)) return // Special case: the interactive popover is the bookmark "add" dialog
  // if ((document.querySelector('.material-bookmark .folder') as any)?.contains(e.target)) return // Special case for quick-closing on a bookmark folder click
  if ((e.target as HTMLElement)?.closest('.material-bookmark .folder')) return // Special case for quick-closing on a bookmark folder click
